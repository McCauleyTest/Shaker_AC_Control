...
const int TEMPLOW = 60;     //the low temp point where air conditioners will turn off
const int TEMPHI = 63;      // the high temp point where air conditioner will turn on
const int SHORTCYCLE = 60 * 1000;  //short cycle delay to allow compressor time to depressurize
...
void loop(void) 
{
 ...
 for (a=0; a<2; a++){
  ...
  float tempin = map(steinhart[0], 20, 120, 0, 255);
  float tempout = map(steinhart[1], 20, 120, 0, 255);
